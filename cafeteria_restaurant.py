...
class DashboardApp:
    ...
    def completar_pedido_cocina(self, ubicacion_key):
        """Marca el pedido como completado en cocina, descuenta automáticamente los ingredientes del inventario y lo retira de la vista[cite: 6]."""
        if ubicacion_key in self.pedidos_por_ubicacion:
            ...
            if db:
                try:
                    ...
                    for item in pedido.get("items", []):
                        ...
                        ingredientes = self.obtener_ingredientes_plato(nombre_plato)
                        for ing in ingredientes:
                            cant_a_descontar = float(ing['cantidad_requerida']) * cant_plato
                            # Descontar de la tabla inventario_alimentos conectando automáticamente
                            sql_update = """
                                UPDATE inventario_alimentos 
                                SET cantidad = GREATEST(0.00, cantidad - %s) 
                                WHERE nombre = %s
                            """
                            cursor.execute(sql_update, (cant_a_descontar, ing['nombre']))
                    
                    db.commit()
                    messagebox.showinfo(
                        "Inventario Actualizado", 
                        f"La orden de [{ubicacion_key}] ha sido despachada.\n¡Los ingredientes fueron descontados automáticamente del inventario[cite: 6] para evitar descuadres!"
                    )
                except Error as e:
